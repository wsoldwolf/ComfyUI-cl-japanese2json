...
- **長い入力の翻訳に時間がかかる**: 文書全体のディレクティブと箇条書き境界を索引付きプレースホルダへ、参照タグと日本語発話を短い保護プレースホルダへ置換し、残った通常文章を一つの `translation_stream` 文字列として送信します。実効コンテキスト長へ収まる場合、文書全体を1回の推論で翻訳します。固定の区間件数上限はありません。収まらない場合だけ箇条書き境界で最小限に分割し、`Translating batch X/Y` を進捗として表示します。
- **翻訳検証失敗**: 構造プレースホルダの個数と順序、区間ごとに一意な保護プレースホルダ、区間外への文章追加、コードフェンス、thinking、日本語残留などを検出しています。元のディレクティブ、参照タグ、`<d>` 発話はPython側の対応表から復元します。内部JSONは入力に単一の `translation_stream`、応答に単一の `translation` だけを使用し、余分なキーを許可しません。初回応答で正常だった区間は保持し、失敗した区間だけを具体的な検証理由付きで1回再試行します。再び失敗した場合は不正JSONを返さずノードエラーになります。
...
- **モデルが制約を繰り返し破る**: `abliterated` 版より、同規模の標準的なinstruction-tuned GGUFを優先してください。モデル変更は翻訳速度よりも、JSON形式と保護トークンの追従性改善を目的とします。
- **Qwen3のthinkingが混入する**: 本ノードはユーザーメッセージ末尾の `/no_think` と単一翻訳ストリーム用JSON modeを併用します。system promptでも全プレースホルダを翻訳、改変、移動、複製または削除しないよう明示しています。thinkingを除去して採用することはなく、混入時は検証エラーになります。
