...
各項目は1回までです。環境音、効果音及び生成用BGMは任意文又は`なし`、発声は`なし`又は`指定台詞のみ`だけを使えます。環境音、効果音及び生成用BGMへ`<Audio N>`や台詞は書けません。既存Audioは`BGM再利用: <Audio N> 完全コピー|部分コピー [MM:SS.mmm-MM:SS.mmm]`でだけ指定します。時間範囲は`部分コピー`でだけ使用できます。
