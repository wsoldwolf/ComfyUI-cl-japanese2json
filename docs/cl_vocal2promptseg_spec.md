...
Lyrics整列による有声昇格後、隣接する同一状態の範囲を再結合してから同じ均等分割を再実行する。後続Sceneを持つ1秒Sceneは、標準の22フレーム継続コンテキストに対して再利用可能映像が17フレームしかなくContex-Loopで停止するため出力しない。1秒の無音範囲は隣接する有声範囲へ安全側に吸収し、孤立した1秒の有声範囲は隣接する無音範囲を1秒だけ有声側へ拡張して2秒以上にする。これにより実際の有声PCMを無音Sceneへ分類せず、総尺、先頭時刻、末尾時刻及びSource Timelineの連続性を維持する。最終Sceneだけは後続へ映像コンテキストを渡さないため1秒を許容する。

ここで生成するScene秒数は完成動画上のSource Timeline範囲であり、H3へ渡すraw生成尺ではない。後段の`CL Japanese to JSON (GGUF)`は、`anchor_mode=head`で各継続Sceneから除去されるcontextを`length`へ補い、24fpsの累積配信尺がテンプレート全体を覆うようH3フレーム格子上で誤差を分散する。したがって本ノードはcontext分をScene秒数へ加算せず、`// ソース範囲`も変更しない。
...
- 各`# シーン`ディレクティブの直前へ、文書内の出力順に1から始まる連番を`// シーン N`として出力する。これは人間がコンパイラの`Scene N`エラーと入力箇所を照合するための診断コメントであり、Sceneの意味、継続関係、翻訳又はJSONを変更しない。
