...
それでも応答先頭へ正常に閉じた`<think>...</think>`が出た場合は、連続する先頭thinking blockだけを応答エンベロープとして除去できる。翻訳区間内のthinkingを無条件削除してはならない。区間内へ混入したthinkingはその区間だけを検証失敗とし、他の検証済み区間は保持する。未閉鎖thinking又はthinkingしか含まない応答は検証失敗である。
...
初回失敗後、検証済み区間を保持して未解決区間だけを再送する。応答全体に不正thinkingがあった場合も、thinkingを含まない正常区間は保持し、混入区間だけを未解決とする。再試行seedは初期seedと試行番号から決定論的に変える。
