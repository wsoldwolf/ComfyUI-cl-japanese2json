...
- 構造プレースホルダの一部が残る応答を、行数だけで復旧してはならない。構造プレースホルダが完全に0件の場合に限り、翻訳対象が2区間以上、文書内に保護プレースホルダが1個以上あり、非空出力行数が区間数と完全一致するとき、出力行順を区間順として検証してよい。この場合も各行が所有する保護プレースホルダ、別区間からの移動、Subject文末及び日本語残留を通常どおり検証し、1項目でも失敗すれば採用しない。
