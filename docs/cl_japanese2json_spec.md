...
意味が一意な固定プロンプト用語は、`docs/cl_prompt_term_dictionary_spec.md`で定義する外部CSV辞書を用いてLLM送信前にPythonで英語へ正規化する。同梱辞書は`node_japanese_to_json/dictionaries/prompt_terms.csv`とし、`画面中央`、`画面上端`、`画面下端`、`画面全体`、`画面左`、`画面右`、`画面外`、`画面内`、`画面端`及び単独の`画面`を初期値として収録する。LLMが応答へ同じ日本語を再導入した場合も通常検証前に同じ辞書を適用する。既存`<d>`及び日本語かぎ括弧から保護された台詞内部には適用しない。
