...
構造検証後の訳文に対する`semantic_guard`、同一原文の訳文共有及び上限付き局所修復は[意味・情景保全仕様](prompt_semantic_preservation_spec.md)で定義する。
...
箇条書き順に`<Subject 1>`、`<Subject 2>`を割り当てる。Subjectの導入説明は、後続の`subject_definitions`で`<Subject N> is ...`の右辺として使える単数形の英語名詞句で開始し、ASCIIピリオドで終える。後続する属性説明、保持条件及び禁止文はそれぞれの意味を保持した英文として続ける。禁止された外観を名詞句へ取り込んで指定外観へ反転させない。出力は一つのSubjectにつき元の一バレットを維持する。
...
- Source Timeline駆動形式はScene単位の結合とする。縮小Markdownでは重複を避けるため最初のShotへ一行だけ記述してよいが、JSON生成時は同じSubjectのSource Vocal同期をScene内の全Shotへ展開し、Shot境界で口形駆動を失わない。
...
参照タグ及び台詞を保護した後、Subject及び保持分析のバレットを日本語の文末記号`。！？`で内部の翻訳区間へ分割する。小数点、英語の略語及び保護済み台詞内の句読点は分割対象にしない。各区間には元バレットIDと文番号を保持し、文に実在する保護トークンだけを割り当てる。翻訳後は元の順序で空白結合し、一つのSubject及び保持分析につき一バレットという外部仕様を維持する。Common、Scene、Shot及び音響の本文は、許可する動作に続けて例外や禁止を指定する局所的な文脈を保つため、元のバレットを一つの翻訳区間とする。Scene、Shot、音響の構造及びバレット数も変更しない。

明示的な禁止を含む区間と、複数文で構成されたSubject・保持分析の各文は、一つずつ独立した推論要求にする。小型モデルが隣接マーカーへ属性や否定を移しても構文だけでは検出できないためである。その他は最大16区間を実効コンテキストに応じてバッチ処理する。一つの文そのものを文字数や読点で途中分割しない。禁止文が多い入力では推論回数が増えるが、再翻訳は未解決区間に限定する。
...
- SUBの導入説明は`<Subject N> is`の右辺となる単数名詞句で開始し、後続の制約文は肯定・否定を保持した英文にする。
...
- 検出対象の明示的な禁止文に、英語の否定又は禁止表現が残る。

否定脱落検査は`translation_units.py`の文法パターンによる限定的な形式検査である。`しない`、`せず`、`させない`等を含む対象文は、`not`、`without`、`avoid`等の明示的な表現を保持するよう要求し、脱落時は既存の`retry_max`内でその文だけ再翻訳する。少ない・危ない等の形容詞を一律に否定と判定しない。`しないといけない`等の義務表現や検出対象の二重否定は通常翻訳に委ねる。保護された台詞内の否定は本文の検査に使用しない。応答の部分回収及び参照復元後も同じ検査を行い、未解決の訳文を採用しない。

この検査は意味の同一性を保証しない。禁止の対象・範囲、複数禁止事項の一部脱落、未対応の文法、別の否定句へ意味が移る誤訳を完全には判定できない。システムプロンプトで禁止対象と後続の肯定指示の分離を指定し、独立翻訳で文間混同を抑制する。キャラクター名や身体部位等をPythonの固定翻訳辞書へ追加して対処しない。

応答本文はthinking、コードフェンス、長さ上限等の応答全体検証より前に保持する。応答全体の検証に失敗しても、構造マーカーで安全に分離できる区間を個別検証し、正常な区間を回収する。未閉鎖thinking又は余分な文章が最初の構造マーカーより前にある場合、その前置きはどの翻訳区間にも属さないため破棄し、マーカーで境界が確定する後続区間を個別検証する。余分な前置きだけを理由に正常なバッチ全体を再送してはならない。重複、欠落又は順序不正の構造マーカーがある場合は、そのマーカーによって開始又は終了境界が曖昧になる区間だけを未解決とする。
...
参照又は台詞の保護トークンが欠落・重複・他区間へ混入した場合は`ProtectedPlaceholderError`として識別する。ComfyUIの`LlamaBackend`では、当該未解決区間を次の再試行からGBNF付き生成へ切り替える。既存の検証済み区間は再送せず、制約が必要な区間と通常再試行の区間は別要求にする。構造エラーで通常解析できない応答も、保護トークン数が不正な未解決区間は同様に切り替える。`retry_max=0`では切り替えるための追加推論を行わず、有限・無制限の既存設定及び中断機能を尊重する。

`stream_grammar.py`は対象ストリームだけからGBNFを生成する。構造マーカー、区間順序、各区間の保護トークンの出現順序と各1回の出現、改行及び終端を必須とする。自由本文では予約接頭辞`CLJ`を生成できないため、同じタグの複製や別区間への混入を許さない。参照・台詞本文は文法へ埋め込まず、保護トークンだけを使用する。日本語と英語の語順差はトークン前後の自由本文で表現し、意味の翻訳はLLMが担当する。参照キーのメタデータを要求へ添え、由来・対象・所有関係を保持させる。文法対応バックエンドでは、欠落タグを後から文頭や文末へ推測挿入する既存救済は使用しない。

文法は`LlamaGrammar.from_string(..., verbose=False)`で作成し、通常chat completion及びQwen3のnon-thinking text completionへ渡す。未対応又は生成失敗を黙って無視しない。`compile_grammar`を持たない外部テスト用・互換バックエンドでは従来の厳密な応答検証と再試行を使用する。制約使用時も応答長、否定、残留日本語及びタグ数の独立した検証をすべて維持する。GBNFは形式の保証であり、意味の同一性や生成の完了を保証しない。デバッグイベントには`transport`（`validated_stream`又は`gbnf`）及び実際の`grammar`を記録する。

再送時も同じ未解決グループが2回連続で一件も減らない場合は、意味を推測して結果を書き換えず、現在のグループ上限を半分へ縮小する。最小値は1レコードとする。縮小グループで検証できた結果を保持し、残りを同じ上限で順次処理する。個々の検証エラーには、保護台詞外へ残った日本語の連続文字列を最大8件含め、再送要求でその具体的な残留語をモデルへ通知する。英語文中へ孤立した日本語語句が残る場合も検証失敗とする。

意味が一意な固定プロンプ用語は、`docs/spec/cl_prompt_term_dictionary_spec.md`で定義する外部CSV辞書を用いてLLM送信前にPythonで英語へ正規化する。同梱辞書は`node_japanese_to_json/dictionaries/prompt_terms.csv`とし、フレーム位置を表す`画面中央`、`画面上端`、`画面下端`、`画面全体`、`画面左`、`画面右`、`画面外`、`画面内`、`画面端`及び単独の`画面`に加え、可読性の意味極性を固定する`非可読`を初期値として収録する。疑似文字の表示を許可する複合語は収録しない。LLMが応答へ同じ日本語を再導入した場合も通常検証前に同じ辞書を適用する。既存`<d>`及び日本語かぎ括弧から保護された台詞内部には適用しない。

利用者は`ComfyUI/user/cl_japanese2json/prompt_terms.csv`で対応を追加し、同一`source`を上書きできる。辞書は複合語優先で適用し、ファイル変更を次の正規化時に再読込する。CSV不正又は日本語を含む`target`を黙って無視してはならない。固定辞書を一般の日本語語彙へ無制限に拡張して自由文を推測翻訳してはならない。

最初の出力チャンクが120秒得られない場合、又は1個以上の出力チャンクを受信した後45秒新しいチャンクがない場合は、応答検証を待たず`InferenceStallError`とする。対応するllama-cpp-pythonでは`llama_set_abort_callback()`を使用してネイティブdecodeへ停止を要求し、Pythonの停止理由を上位へ復元する。停滞した未解決グループはレコード数を半分へ縮小して新しいseedで再試行する。最小値は1レコードとする。縮小後のグループが成功した場合、検証結果を保持して残りの未解決レコードだけを同じ上限で順次処理し、この後続処理自体は再試行回数を消費しない。

再送ストリームでは、参照タグを表す不透明な`CLJ...X`の直後へ、対応する`(<Subject N>)`、`(<Picture N>)`、`(<Video N>)`又は`(<Audio N>)`を一時的な意味注釈として付ける。これにより、モデルが自然な英語で参照を代名詞へ省略することを抑える。検証時は、CLJトークンが維持されていれば注釈を除去し、CLJトークンの代わりに正確な注釈だけが返った場合は元トークンへ戻す。再試行でも参照が省略された場合、原文で参照トークンが文頭、又は`最初に、`、`次に、`、`続いて、`、`最後に、`等の既知の時系列接頭辞の直後にあり、対応する英文の文境界を一意に特定できる場合だけ、その英文の先頭代名詞を参照トークンへ戻すか意味上の文頭へ明示的に補う。任意位置への推測挿入は行わない。日本語台詞は注釈へ展開しない。余分、重複又は対応不能な参照タグは引き続き検証失敗とする。
...
一般のバックエンド例外は検証失敗として無限再試行しない。明示的に回復可能な`InferenceStallError`だけは上記の縮小再試行対象とする。上限到達後は破損結果を返さず`TranslationError`とする。
...
- `SOURCE_VOCAL`行は、同尺のSource Vocalに検出された人声区間、音素、閉口、持続音及びフレーズ境界だけで口形を駆動する固定文へ置換する。Scene内の各Shotへ固定文を出力し、発声区間中は口形を継続的に変化させる。無声区間では口を閉じ、Shot又はScene境界でフレーズを再開始せず、歌詞を推測しない。
...
通常ログはモデル探索・ロード設定、ディレクティブ数、翻訳区間数、バッチ数、トークン数、再試行、アンロード、生成Scene数を記録する。最終JSONの検証完了後、出力を返す直前に`[cl_japanese2json] success: generated N scene(s)`をANSIシアン色で記録する。`keep_last_prompt=True`によるキャッシュ返却も`success: returned cached last JSON`として記録する。プロンプト全文は通常ログへ出さない。
...
各推論の実行中は10秒間隔のheartbeatをINFOで記録する。heartbeatはバッチ、attempt、経過秒、受信済みcontentチャンク数、最終チャンクからの経過時間及び進捗callback実行中かどうかを含み、最初のチャンク以前は0件とする。これにより、モデルから次チャンクを待っている状態とUI進捗通知内で待っている状態を区別できる。最初のチャンク前120秒又は受信開始後45秒の無進展は停滞として中断・縮小再試行する。応答又は例外時にheartbeatを停止し、入力又は生成途中の本文を通常ログへ含めない。
