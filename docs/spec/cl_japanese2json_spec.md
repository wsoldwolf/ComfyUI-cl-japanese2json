...
実効コンテキスト長に文書全体が収まり、翻訳レコード数が16以下の場合、推論要求は1回である。17レコード以上又は実効コンテキストへ収まらない場合は、レコード境界で1バッチ最大16レコードに分割する。1レコードを途中分割しない。上限は、長い単一生成の停滞及び全体再試行の負担を抑えつつ、一行単位推論の初期化オーバーヘッドを避けるための固定値である。
