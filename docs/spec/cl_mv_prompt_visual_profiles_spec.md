...
同プロファイルでは`lyric_action_preplan=true`及び`lyric_action_scenes_per_request=1`とし、Scene本体より先に`clmv-lyric-action-line-v1`を歌詞Scene一件ずつ実行する。この入力にはPlanningBriefのSubject定義と保持分析、及び対象Sceneの`lyric_lines`だけを渡す。共通プロンプトの画材・背景、Song Bible、section motif、直前Scene本文、カメラ及びShot構造は渡さない。まず歌詞の主体、中心動詞、具体的対象、接触経路及び完了結果を固定し、画風と背景は後段で適用する。応答は歌詞アンカー、方式、接触前の構図要件、二～四個の時系列ACTION及び可視結果だけを持つ。Pythonは各レコードを独立検証し、不正又は欠落Sceneだけを再試行する。8Bが内部Subjectトークンを既知の`<Subject N>`へ正確に戻した場合に限り、参照凡例を使って再保護し、未知Subject又は他種参照は拒否する。人物の各時系列ACTIONは腕、手、脚、足、胴体又は全身支持を使う明瞭な関節動作とし、髪、耳、尾、袖、視線、口又は表情だけを主動作にしない。身体段階の不足だけを補う場合は、高速な全身滑走や微小なその場姿勢変更ではなく、足の接地と重心移動を伴う踏み出し、腕、胴体又は身体レベルの変化を追加する。

検証済みpreplanは`locked_lyric_action_blueprint`としてScene要求の末尾へ渡す。Scene LLMは構図、環境、カメラ及び付加映像をこの対象・接触・結果へ適合させる。Pythonは解析後、Scene LLMが返したACTIONをblueprintの時系列ACTIONで置換して複数Shotへ順序を保って分配し、可視結果を最終Shotの最後へ追加する。最終Shotを可視結果だけにせず、blueprintよりShot数が多い場合は当該Shotの生成済みSubject動作を一件保持する。また対象が接触前から認識できるよう各ShotのCOMPOSITIONへ構図要件を保持する。BlueprintがあるSceneではアンカー、応答方式及びSubject結合も検証済み値を優先する。Scene LLMがこれらを誤記したり、生成ACTIONでSubjectトークンを人物名へ展開したりしても、置換直前の一時値だけを理由に再試行しない。Shot数、フィールド順、カメラ契約又は付加映像等の置換されない構造違反は従来どおり再試行する。これにより、共通画材、section motif又は`previous_scene_tail`の複写が歌詞の中心動詞を消すことと、小型モデルが同じ表層エラーでretry上限を消費することを防ぐ。27B向け`lyric_visuals_full`及び`performance_only`は`lyric_action_preplan=false`で従来の単一Scene計画を維持する。
