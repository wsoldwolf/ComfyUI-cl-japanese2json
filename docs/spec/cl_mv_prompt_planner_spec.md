...
固定条件はPythonが解析した`PlanningBrief`だけを正本とし、Song Bibleへ再生成させない。Song Bible v4、確定条件と創作情報の分離、構造化JSON入力、active motif、直前Sceneの最終状態、重複Scene及び付加映像反復の排除、カメラ意味ガードは`docs/spec/cl_mv_prompt_planner_song_bible_spec.md`を正本とする。視覚拡張プロファイル、`AUX_VISUAL`及び拡張方法は`docs/spec/cl_mv_prompt_visual_profiles_spec.md`を正本とする。

### 5.2 Sceneバッチ

`lyric_action_preplan=true`のプロファイルでは、各Sceneバッチの本計画より前に歌詞Sceneだけを`clmv-lyric-action-line-v1`へ渡す。一回の要求数はプロファイルの`lyric_action_scenes_per_request`で決定し、8B用は1件とする。この小さい行指向段階はカメラ、Song Bible、section motif及び直前Sceneを扱わない一方、Subject定義、保持分析、`global_visual_direction`及び`scene_anchors`を受け取る。後二者は利用可能な物体・支持面、時間帯、接地及び禁止事項を決める世界制約としてだけ使用し、画材、照明、背景又はカメラの文章をACTIONへ転記しない。その制約下で歌詞の主体、中心動詞、対象、接触経路及び結果を二～四個のACTIONへ分解する。正常レコードを保持して不正又は欠落Sceneだけを再試行する。引用された歌詞、発話、口の動き又は画面内文字を生成した応答では、Pythonが当該`LYRIC_RESPONSE`の歌詞番号を取得する。代替歌詞行が存在すればその番号を`excluded_lyric_indices`へ追加し、元の一始まり番号を維持したまま次回の`lyric_lines`から除外する。再試行は残る最初の非発話述語を選び、引用符だけを除去した同一解釈を再提出してはならない。代替行がない場合は、同じ歌詞を引用、口の動き及び可読文字のない非言語的な空間又は物体動作として再解釈する。小型モデルが既知の内部Subjectトークンを正確なユーザー表記`<Subject N>`へ復号して返した場合だけ、参照凡例との完全一致を確認して保護トークンへ戻し、WARNING付きで受理する。未定義Subject、`Picture`、`Video`、`Audio`等の生タグ及び一般Scene応答の生タグは従来どおり停止エラーとする。

preplan結果は`locked_lyric_action_blueprint`として本計画へ渡す。本計画は構図、環境、カメラ及び付加映像をblueprintへ適合させる。Pythonは複数Shotへblueprintの時系列ACTIONを分配した後、本計画が共通プロンプト、利用可能な環境及びカメラ軌道に合わせて生成した、同じSubjectを含む重複しない補助ACTIONを保持して結合する。各Shot内ではNFKC、空白、末尾句点、Subjectに続く「の／は」及び否定・反復を含まない文の「ゆっくり」の差を正規化して重複を除去する。左右・否定・対象・明示的な反復指示は保持し、曖昧な意味類似度で削除しない。本計画が割り当て済みの中心動作を同じ順序で正確に再出力した場合は、その前後にある準備と立て直しの配置を利用する。追加する補助ACTIONは中心動作と合わせ原則4行までとし、中心動作そのものを上限のために切り捨てない。可視結果は別枠として最終Shotの最後へ固定し、後段の不足補完も結果より前へ挿入する。一Shotは最大8 ACTION、最終Shotでは結果用に一枠を予約する。必要対象は各COMPOSITIONへ保持する。これにより歌詞由来の中心動詞を失わず、同時に従来のように本計画の予備動作、追従、反動及び立て直しを全削除しない。Blueprintが存在するSceneでも、統合後の最終ACTION列に対して長さ別の身体動作検証と必要なACTION限定LLM修復を実行する。Scene LLMがSubjectトークンの代わりに人物名を書いた場合やアンカー・方式を誤記した場合でも、後段で固定されるフィールドだけを理由にScene全体を再試行せず、Shot、構図、環境、カメラ及び付加映像の構造検証を継続する。この段階を無効にしたプロファイルの要求回数と生成経路は変えない。
...
各batchは独立したチャット要求とし、暗黙の会話履歴へ依存しない。世界の継続性はSong Bible、active_section_motifs及びscene_anchorsを明示して維持する。`previous_scene_tail`には直前の検証済みSceneの内部scene_id、Subject参照及びカメラのtype/amplitude/speedだけを含める。最終構図、最終ACTION、環境、付加映像及びカメラ説明原文は渡さない。これにより眉の振動等の演技を後続Sceneへ複写する経路を減らす。ただし終端の正確な姿勢・接触状態を継承する機構ではないため、各Sceneは現在の対象と支持状態から実行可能な演技を記述する。同一応答内の後続Sceneも、前の演技を必要な範囲で収束させ、現在の歌詞に合う新しい演技へ進む。内部Scene番号は出力本文へ転記しない。部分再試行時はSceneごとに継続情報を再計算する。重複判定にはACTION修復前のfingerprintを使用し、完全重複のSceneへ不足動作の修復を先に行わない。完全重複又は別Sceneと完全一致するSubject ACTION列を検出したbatchは一件ずつの再送へ切り替え、`avoid_duplicate_plans`と`duplicate_repair`で構造差を促す。
...
Subjectを持ちPlanningBriefが全身の完全静止を明示していないSceneでは、ワールド位置、重心、胴体、支持脚又は手足の構成を可視的に変える意図的身体動作を要求する。受動的な浮遊、呼吸、衣装、髪、粒子、表情、口、視線及びカメラだけの運動は数えない。必要段階数はScene全体と各Shotの長さに対して、それぞれ4秒以下は1、5～7秒は2、8～11秒は3、12秒以上は4とする。同じ動作の言い換えを段階として重複計上せず、左右・対象・否定・明示的な反復は区別する。歌詞事前計画のdirect_subject_actionに身体段階が一つもない場合、又は明示的な唇・舌の振付、口内の発光、眉の上下・振動がある場合は固定前に拒否する。代替歌詞があれば次の候補を選ぶ。一つ以上の身体段階を持つBlueprintの不足は本計画へ渡して補う。

最終統合後に身体段階が不足した場合は、同じLLMへ`clmv-motion-repair-v1`でACTIONだけの修復を要求する。修復対象には歌詞・共通制約、既存の構図・環境・カメラ・ACTION、Shotごとの必要段階数を渡す。各SceneのACTION修復はScene再試行を通算して最大2回とする。Shotの時刻・数・順序、歌詞アンカー及びACTION以外のフィールドはPythonで不変に保ち、各Shotの固定歌詞ACTIONの順序と最終VISIBLE_RESULTも検証する。返されたACTIONには参照・構文・身体段階・顔の振付・カメラ混入・ボーカル整合性の検査を適用する。修復不成立時は無効な応答を採用せず、既存のScene再試行予算を使う。本経路では屈伸や腕振りの定型補完を行わない。単独パーサーの互換用repair_motion=Trueは残すがPlannerはFalseを使用する。採用された修復は`planner_json.metadata.motion_repairs`へmethod=targeted_llmと試行数を記録する。人物を持たない物体・抽象Sceneは身体段階検査の対象外とする。規則ベース検査は自然さの完全な意味判定ではない。

修復応答は`MOTION_REPAIR<TAB>scene_id`で開始し、既存Shotごとに`SHOT<TAB>start_ms`、1始まりの連番を持つ1～8行の`ACTION<TAB>index<TAB>text`、`END_SHOT`を記述し、`END_MOTION_REPAIR`で終了する。他のフィールドやShot追加は受理しない。

修復応答で先頭のSubject参照と直後の助詞（の／は／が）だけが欠落した場合、同じShotの元ACTIONからその接頭辞を除いた本文との一意な一致を確認して元の文を復元する。固定VISIBLE_RESULTも元ShotのACTIONに含まれる場合は同じ規則を使う。複数Subjectへの曖昧な一致、動作・対象・左右・否定・速度の書き換えは復元しない。復元後も固定動作の順序、最終結果、身体段階及び既存の安全性検査を適用する。「姿勢を取る／とる」「手を添える」を意図的動作に含め、姿勢を保持するだけの文とは区別する。

修復用system promptには対象Scene番号と全Shotの開始時刻を数値で埋め込み、Sceneの開始・終了は全Shotを囲む各一行に限定する。`requested scene_id`等の説明用文字列を出力例として提示しない。Scene全体及びShotごとの必要段階数を要求へ含め、本計画にもShot長別の下限を通知する。手足を前へ出す、接地させる、接触対象から離す動作も身体段階として数える。修復上限到達時は最後の修復エラーを保持してSceneの不足診断と併記し、各試行のdebug requestに当時のretry_feedbackを保存する。

地面に接するSubjectのワールド位置を変更するACTIONは、支持面との接触、重心移動、踏み出し又は同等の関節運動による推進及び安定した着地を示す。PlanningBrief又は現在歌詞が飛行若しくは浮遊を明示しない限り、外観を固定した身体全体を足や支持点の運動なしに画面内へ滑走又は浮遊させてはならない。身体段階の件数を満たすだけの無関係な移動より、その場での身振り又は姿勢変化を優先する。
...
10秒以上の歌詞Sceneは、選択プロファイルが指定するShot数範囲を使用する。`lyric_visuals_light_8b`は正確に2 Shot、`lyric_visuals_full`は2～3 Shotとし、予備動作、接触及び結果の意味的遷移へ境界を置く。8秒以上の複数Shot Sceneでは、等分位置を中心として一区間長の35%を前後許容した`balanced_shot_start_windows_ms`を渡す。LLMは窓内の意味的遷移を選び、等分値そのものへ機械的に固定しない。構造的に有効な応答が窓外だった場合、PythonはScene再推論を行わず、当該開始時刻だけを最寄り境界へ補正してWARNINGを出す。これにより14秒Sceneの第2 Shotが13秒から始まる等、片側へ時間が偏る計画を防ぐ。後続Shotは、物理的に不可欠な場合を除きカメラ種別を変え、角度、距離、奥行き関係又は提示情報も変化させる。視点変更時も同じSubject、身体、衣装及び物体を維持し、カメラ移動を変身又は複製として扱わない。
...
- 完全一致する別SceneのSubject ACTION列がretry上限まで解消しない場合
