...
preplan結果は`locked_lyric_action_blueprint`として本計画へ渡す。本計画は構図、環境、カメラ及び付加映像をblueprintへ適合させる。Pythonは複数Shotへblueprintの時系列ACTIONを分配した後、本計画が共通プロンプト、利用可能な環境及びカメラ軌道に合わせて生成した、同じSubjectを含む重複しない補助ACTIONを保持して結合する。各Shot内ではNFKC、空白及び末尾句点を正規化した文単位の完全一致だけを除去する。左右・否定・対象・明示的な反復指示は保持し、曖昧な意味類似度で削除しない。本計画が割り当て済みの中心動作を同じ順序で正確に再出力した場合は、その前後にある準備と立て直しの配置を利用する。追加する補助ACTIONは中心動作と合わせ原則4行までとし、中心動作そのものを上限のために切り捨てない。可視結果は別枠として最終Shotの最後へ固定し、後段の不足補完も結果より前へ挿入する。一Shotは最大8 ACTION、最終Shotでは結果用に一枠を予約する。必要対象は各COMPOSITIONへ保持する。これにより歌詞由来の中心動詞を失わず、同時に従来のように本計画の予備動作、追従、反動及び立て直しを全削除しない。Blueprintが存在するSceneでも、統合後の最終ACTION列に対して長さ別の身体動作検証と必要な決定論的補完を実行する。Scene LLMがSubjectトークンの代わりに人物名を書いた場合やアンカー・方式を誤記した場合でも、後段で固定されるフィールドだけを理由にScene全体を再試行せず、Shot、構図、環境、カメラ及び付加映像の構造検証を継続する。この段階を無効にしたプロファイルの要求回数と生成経路は変えない。
...
`ACTION`にSubjectが現れ、PlanningBriefが全身の完全静止を明示していない場合、少なくとも一つの`ACTION`は当該`<Subject N>`を明示し、ワールド位置、重心、胴体の向き、身体レベル、姿勢又は手足の構成を可視的に変化させる。受動的な浮遊、上下動、揺れ、呼吸、衣装、髪、粒子、表面エフェクト、表情、口、視線又はカメラだけの運動を身体動作として数えない。必要な意図的身体段階数はScene長に応じて、4秒以下は1、5～7秒は2、8～11秒は3、12秒以上は4とする。段階は序盤へ集中させず、予備動作、主動作、反動及び支持された立て直しとしてScene全体と最終Shotまで配分する。副詞、方向又は対象が身体部位と動詞の間に入る「右手を頭上までゆっくりと上げる」等も身体動作として認識する。保持対象の顔部品、眼窩、身体輪郭又は衣装層を移動・変形する記述は身体動作として数えない。Pythonは同じ数値を`subject_motion_contract`としてpreplanとScene入力へ渡す。`lyric_action_preplan`を使用する場合は、構文、参照及び歌詞アンカーが有効なBlueprintを保持し、身体段階だけが不足しても、この段階では補完文を追加せず本計画へ渡す。この不足だけを理由としてBlueprintを再推論してはならない。本計画との最終統合及び重複除去の後に一度だけ身体段階を検証し、必要最小限の補完を実行する。これにより補完文が本計画に模倣され、再統合で増殖する経路を断つ。単独パーサーの互換用repair_motion=Trueは従来の即時補完を維持するが、Planner本経路はFalseを使用する。Blueprintを使用しないSceneで不足した場合も、LLMが作成した構図、ACTION、環境及びカメラを保持し、現在の形状と外観及び支持状態を維持したままSubjectの姿勢を変える媒体・歌詞・解剖非依存のACTIONを不足数だけ決定論的に追加する。補正はワールド位置の高速移動としてはならず、支持点を滑らせない。補正内容はWARNING及び`planner_json.metadata.motion_repairs`へ記録する。物体だけのScene及び人物を持たない抽象cutawayは、ACTIONにSubjectを記載しない限りこの検査の対象外とする。
