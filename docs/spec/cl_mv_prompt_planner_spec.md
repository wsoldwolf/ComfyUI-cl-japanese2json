...
preplan結果は`locked_lyric_action_blueprint`として本計画へ渡す。本計画は構図、環境、カメラ及び付加映像をblueprintへ適合させ、Pythonは解析後のACTION列を検証済みblueprintで決定論的に固定する。複数Shotでは時系列ACTIONを先に分配し、可視結果は最終Shotの最後へ付加する。可視結果だけのShotを作らず、blueprintの段階数よりShot数が多い場合は同じblueprintを前提に生成された当該ShotのSubject動作を一件保持する。必要対象は各COMPOSITIONへ保持する。したがって8Bが共通画材又は直前Sceneを複写しても、歌詞由来の中心動詞は最終Markdownから消失しない。Blueprintが存在するSceneでは、その歌詞アンカー、応答方式及びSubject結合をScene LLMの一時的なACTIONより先に検証済み値として扱う。Scene LLMがSubjectトークンの代わりに人物名を書いた場合やアンカー・方式を誤記した場合でも、後段で置換されるフィールドだけを理由にScene全体を再試行せず、Shot、構図、環境、カメラ及び付加映像の構造検証を継続する。Blueprintを実際に適用した最終ACTIONがSubject条件を満たすことを前提とする。この段階を無効にしたプロファイルの要求回数と生成経路は変えない。
